...
TriMesh *make_plane(int tess_x, int tess_y)
{
	...
	for (j = 0; j < tess_y+1; j++) {
		float y = -1.0f + 2.0f * j / tess_y;
		for (i = 0; i < tess_x+1; i++) {
...
void usage(const char *myname)
{
	...
	fprintf(stderr, "	plane m [n]	m x n tesselated square (default n = m)\n");
	fprintf(stderr, "	bump n sigma	n x n tesselated Gaussian bump of width sigma\n");
	fprintf(stderr, "	wave n omega	n x n tesselated sine wave of frequency omega\n");
	fprintf(stderr, "	frac n		n x n fractal landscape\n");
	fprintf(stderr, "	cube n		n x n tesselated cube\n");
	fprintf(stderr, "	disc n m	Circular disc, tesselated with m rings of n points\n");
	...
	fprintf(stderr, "	sphere n m	Sphere, tesselated in polar coordinates\n");
