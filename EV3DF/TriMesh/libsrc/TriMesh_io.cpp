...
static TriMesh *read_off(FILE *f)
{
	...
	if (!read_verts_asc(f, newmesh, nverts, 3, 0, -1, -1, -1))
		goto readerror;
	if (!read_faces_asc(f, newmesh, nfaces, 1, 0, 1, true))
		goto readerror;
...
static TriMesh *read_sm(FILE *f)
{
	...
	if (fscanf(f, "%d", &nfaces) != 1)
		return newmesh;
	if (!read_faces_asc(f, newmesh, nfaces, 0, -1, 0))
		goto readerror;
	...
	int nfaces, int face_len, int face_count, int face_idx)
{
	...
	int old_nfaces = newmesh->faces.size();
	int new_nfaces = old_nfaces + nfaces;
	newmesh->faces.reserve(new_nfaces);
...
static void mkface(TriMesh *mesh, std::vector<int> &grid, int v1, int v2, int v3)
{
