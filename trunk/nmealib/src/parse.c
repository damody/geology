...
/**
 * \brief Parse DBT packet from buffer.
 * @param buff a constant character pointer of packet buffer.
 * @param buff_sz buffer size.
 * @param pack a pointer of packet which will filled by function.
 * @return 1 (true) - if parsed successfully or 0 (false) - if fail.
...
NEMA_PARSE_FUNCTION(GPAAM)
{
	NEMA_PARSE_CHECK(GPAAM);
	return 0;
}
NEMA_PARSE_FUNCTION(GPBOD)
{
	NEMA_PARSE_CHECK(GPBOD);
	return 0;
}
NEMA_PARSE_FUNCTION(GPBWW)
{
	NEMA_PARSE_CHECK(GPBWW);
	return 0;
}
NEMA_PARSE_FUNCTION(GPGLL)
{
	NEMA_PARSE_CHECK(GPGLL);
	return 0;
}
NEMA_PARSE_FUNCTION(GPMSK)
{
	NEMA_PARSE_CHECK(GPMSK);
	return 0;
}
NEMA_PARSE_FUNCTION(PGRME)
{
	NEMA_PARSE_CHECK(PGRME);
	return 0;
}

NEMA_PARSE_FUNCTION(SDDBK)
{
	return nmea_parse_SDDBT(buff, buff_sz, pack);
}
NEMA_PARSE_FUNCTION(SDDBS)
{
	return nmea_parse_SDDBT(buff, buff_sz, pack);
}
NEMA_PARSE_FUNCTION(SDDBT)
{
	int res;
	NEMA_PARSE_CHECK(SDDBT);
	res = nmea_scanf(buff, buff_sz,
		"$SDDBT,%f,%C,%f,%C,%f,%C*",
		&(pack->depth_f), &(pack->uints_f),
		&(pack->depth_M), &(pack->uints_M),
		&(pack->depth_F), &(pack->uints_F));
	if(6 != res)
	{
		nmea_error("SDDBT parse error!");
		return 0;
	}
	if( pack->uints_f != 'f' ||
		pack->uints_M != 'M' ||
		pack->uints_F != 'F' )
	{
		nmea_error("SDDBT parse error (format error)!");
		return 0;
	}
	return 1;
}
NEMA_PARSE_FUNCTION(SDDPT)
{
	NEMA_PARSE_CHECK(SDDPT);
	return 0;
}
NEMA_PARSE_FUNCTION(SDMTW)
{
	NEMA_PARSE_CHECK(SDMTW);
	return 0;
}
NEMA_PARSE_FUNCTION(WIMWV)
{
	NEMA_PARSE_CHECK(WIMWV);
	return 0;
}
NEMA_PARSE_FUNCTION(PGRMM)
{
	NEMA_PARSE_CHECK(PGRMM);
	return 0;
}
NEMA_PARSE_FUNCTION(PGRMZ)
{
	NEMA_PARSE_CHECK(PGRMZ);
	return 0;
}
