...
int nmea_gen_SDDBT(char *buff, int buff_sz, nmeaSDDBT *pack)
{
	return nmea_printf(buff, buff_sz,
		"$SDDBT,%.1f,%C,%.1f,%C,%.1f,%C",
...
void nmea_info2SDDBT(const nmeaINFO *info, nmeaSDDBT *pack)
{
	nmea_zero_SDDBT(pack);

	pack->depth_f = info->depthinfo.depth_f;
	pack->depth_M = info->depthinfo.depth_M;
	pack->depth_F = info->depthinfo.depth_F;
}
    ...
    )
{
	...
	else if(pack_mask & SDDBT)
	{
		nmea_info2SDDBT(info, &dbt);
		gen_count += nmea_gen_SDDBT(buff + gen_count, buff_sz - gen_count, &dbt);
		pack_mask &= ~SDDBT;
	}
        else
            break;
