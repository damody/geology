...
void TriMesh::need_curvatures()
{
	...
	std::vector<float> curv12(nv), weights(nv);
	for (i = 0; i < nf; i++) {
		// Edges
		...
		for (j = 0; j < 3; j++) {
			...
			//w[1][2] += u*v; 
			w[2][2] += v*v;
			vec dn = normals[faces[i][(j+2)%3]] -
				 normals[faces[i][(j+1)%3]];
			float dnu = dn DOT t;
			float dnv = dn DOT b;
			m[0] += dnu*u;
			m[1] += dnu*v + dnv*u;
		...
		float diag[3];
		if (unlikely(!(ldltdc<float, 3>(w, diag)))) {
...
void TriMesh::need_dcurv()
{
	...
	for (i = 0; i < nf; i++) {
		...
		for (j = 0; j < 3; j++) {
			...
			vec dfcurv = fcurv[(j+2)%3] - fcurv[(j+1)%3];
			float u = e[j] DOT t;
			float v = e[j] DOT b;
		...
		// Least squares solution
		float d[4];
		if (unlikely(!(ldltdc<float, 4>(w, d)))) {
			//fprintf(stderr, "ldltdc failed!\n");
			continue;
		}
