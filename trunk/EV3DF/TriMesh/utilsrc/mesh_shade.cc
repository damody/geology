...
void colorbynormals(TriMesh *mesh)
{
	...
	for (int i = 0; i < nv; i++) {
		...
		int r = int(min(max(128.0*(n[0]+1.0), 0.0), 255.99));
		int g = int(min(max(128.0*(n[1]+1.0), 0.0), 255.99));
		int b = int(min(max(128.0*(n[2]+1.0), 0.0), 255.99));
...
void acc(TriMesh *mesh, const char *maxsize_, const char *offset_)
{
	...
	for (int i = 0; i < nv; i++) {
		...
		int c = int(min(128.0 * (tmin + tmax) / maxsize, 255.99));
		mesh->colors[i] = Color(c,c,c);
	}
	delete kd;
}
...
void bdyshade(TriMesh *mesh, const char *nedges_)
{
	...
	for (int i = 0; i < nv; i++) {
		int c = int(255.99f * mesh->flags[i] / nedges);
		mesh->colors[i] = Color(255, c, c);
	}
}
		...
		const char *gamma_)
{
	...
	int nv = mesh->vertices.size();
	for (int i = 0; i < nv; i++) {
		float c[3] = { (float) mesh->colors[i][0] / 255.0f,
			       (float) mesh->colors[i][1] / 255.0f,
			       (float) mesh->colors[i][2] / 255.0f };
		c[0] = pow(c[0] * scale + off, gamma);
		c[1] = pow(c[1] * scale + off, gamma);
		c[2] = pow(c[2] * scale + off, gamma);
		unsigned char r = min(max(int(c[0] * 255), 0), 255);
		unsigned char g = min(max(int(c[1] * 255), 0), 255);
		unsigned char b = min(max(int(c[2] * 255), 0), 255);
		mesh->colors[i] = Color(r,g,b);
	}
}
