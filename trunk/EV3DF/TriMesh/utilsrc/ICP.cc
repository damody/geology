...
public:
	NormCompat(const vec &n_, TriMesh *m_): n(n_), m(m_)
		{}
		      ...
		      float &maxdist, int verbose)
{
	...
	for (size_t i = 0; i < nv1; i++) {
		...
		if (!match)
			continue;
		if (s2->is_bdy((match - (const float *) &s2->vertices[0][0]) / 3))
			continue;
#endif
		o1[i] = 1;
	}
	...
	for (size_t i = 0; i < nv2; i++) {
		...
		if (!match)
			continue;
		if (s1->is_bdy((match - (const float *) &s1->vertices[0][0]) / 3))
			continue;
#endif
		o2[i] = 1;
	}
		   ...
		   int verbose, bool do_affine)
{
	...
	eigdc<3,double>(cov1, eval1);
	eigdc<3,double>(cov2, eval2);

	if (!do_affine) {
		// Just uniform scale
		float s1 = sqrt(eval1[0] + eval1[1] + eval1[2]);
		float s2 = sqrt(eval2[0] + eval2[1] + eval2[2]);
	...
	for (int i = 0; i < 3; i++)
		eigmult<3,double>(cov1, eval1, csqrt1[i], csqrt1[i]);
	for (int i = 0; i < 3; i++)
		eval2[i] = sqrt(eval2[i] / n);
	for (int i = 0; i < 3; i++)
		eigmult<3,double>(cov2, eval2, csqrt2[i], csqrt2[i]);
		      ...
		      bool do_scale, bool do_affine)
{
	...
	maxdist = max(sqrt(thresh), 0.5f*maxdist);
	  ...
	  bool do_scale /* = false */, bool do_affine /* = false */)
{
	...
	s1->need_normals();  s2->need_normals();
	s1->need_neighbors();  s2->need_neighbors();
	s1->need_adjacentfaces();  s2->need_adjacentfaces();
	size_t nv1 = s1->vertices.size(), nv2 = s2->vertices.size();
