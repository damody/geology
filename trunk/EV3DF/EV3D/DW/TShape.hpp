...
template < class T > struct TPos
{
	...
	TPos<T> &operator	+=(const TPos<T> &pos)	{ x += pos.x; y += pos.y; return *this; };
	TPos<T> operator-(const TPos<T> &pos) const
	{
		TPos<T> tmp;
		tmp.x = x - pos.x;
	...
	TPos<T> &operator	*=(const T num)		{ x *= num; y *= num; return *this; };
	TPos<T> operator/ (const T num) const
	{
		T fInv =
		T(1)
	...
	TPos<T> &operator				/=(const T num)	       { T     fInv = T(1) /
		num; x *= fInv; y *= fInv; return *this; };
	TPos<T> &operator				=(const TPos<T> &pos)	{ x = pos.x; y = pos.y; return *this; };
	...
	TPos<T> &inverse()	{ x = -x; y = -y; return *this; }
	double length() const
	{
	...
	TPos<T>
	midPoint (const TPos<T> &vec)
	const
	{
		return TPos<T> ((x + vec.x) * 0.5, (y + vec.y) * 0.5);
	}
...
template<class T> struct TRect
{
	...
	TRect<T>	move_x(const T num)	{ left += num; right += num; return *this; };
	TRect<T>	move_y(const T num)	{ top += num; bottom += num; return *this; };
	T GetWidth() const
	{
		return right - left;
	};
	...
	TRect<T> &operator	*=
	(const T num)	      { left *= num; right *= num; top *= num; bottom *= num; return *this; };
	TRect<T> operator/(const T num)
	{
		TRect<T>	tmp;
		tmp.left = left / num;
	...
	bool operator	!=(const TRect<T> &rect)	{ return !operator ==(rect); };
...
template<class T> struct TCircle
{
	...
	TCircle(T x, T y, T _radius) :
	radius(_radius),
	pos(TPos<T> (x, y))
	{ }

	TCircle(TPos<T> &_pos, T _radius) :
	radius(_radius),
	pos(_pos)
	{ }
	...
	TCircle<T> &operator	+=(const TPos<T> &_pos)		{ pos += _pos; return *this; };
	TCircle<T> operator-(const TPos<T> &_pos) const
	{
		return pos - _pos;
	};
	TCircle<T> &operator	-=(const TPos<T> &_pos)		{ pos -= _pos; return *this; };
	TCircle<T> &operator	=(const TCircle<T> &cir)	{ pos = cir.pos; radius = cir.radius; return *this; };
